...
pub mod private_dex {
    ...
    /// Updates the config account with admin
    pub fn update_config(
        ctx: Context<UpdateConfig>,
        is_paused: Option<bool>,
        default_pool_fee_bps: Option<u16>,
    ) -> Result<()> {
        ctx.accounts.update_config(is_paused, default_pool_fee_bps)
    }

    /// Creates a new user state account
    pub fn create_user(ctx: Context<CreateUser>) -> Result<()> {
        ctx.accounts.create_user(&ctx.bumps)
    }

    /// Deposits tokens into user positions
    pub fn deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
        ctx.accounts.deposit(amount)
    }

    /// Withdraws tokens from user positions
    ...
    pub fn create_lp(ctx: Context<CreateLp>) -> Result<()> {
        ctx.accounts.create_lp(&ctx.bumps)
    }
